...
%             the colour control points in rgb. one control point is 
%             necessary for each colour in rgb. exception: when there are 
%             two colours in rgb, three control points can be indicated to
%             represent the middle point. default: all colours are
%             equally spaced
%       
...
%       adjusting the relative values of the color stops, one for each
%       indicated colour:
%       >> pts = [1 5 6 7];
...
function map = multigradient3(rgb, varargin)
...
if strcmp(interp, 'rgb')
    ...
    rgb = setcontrolpoint(rgb, pts);
    map = interp1(pts, rgb, 1:length);
elseif strcmp(interp, 'hsv')
    % converting to HSV
    hsv = rgb2hsv(rgb);
    hsv = setcontrolpoint(hsv, pts);
...
elseif strcmp(interp, 'mshdiv')
    if size(rgb, 1) ~= 2
        ...
        if msh(1,2) > 0.05 && msh(2,2) > 0.05 && acos(dot([cos(msh(1,3)) sin(msh(1,3))],[cos(msh(2,3)) sin(msh(2,3))])) > pi/3
            ...
            % obtaining middle colour hue for both endpoint colours
            if msh(1,1) < Mmid
            ...
            if numel(pts) == 3
                pts = [pts(1), pts(2)-0.0001, pts(2)+0.0001, pts(3)];
            else
                pts = maptorange_local(1:3, [1 3], [1, length]);
                pts = [pts(1), pts(2)-0.0001, pts(2)+0.0001, pts(3)];
            end
        end
        
        msh = setcontrolpoint(msh, pts);

        % interpolating
        mapmsh = interp1(pts, msh, 1:length);

        % converting back to rgb, fixing potential outliers
...
function colours = setcontrolpoint(colours, pts)
...
switch preset
    case 'BlWh' % black-white
        rgb = [0 0 0; 1 1 1];
        interp = 'rgb';
    case 'GrYlRd' % green-yellow-red RGB
        rgb = [0 .8 0; .9 .9 0; .9 0 0];
        interp = 'rgb';
        
    case 'iso.GrRd' % isoluminant green-red
        rgb = [0 .8 0; .8 0 0];
        interp = 'labiso';
    case 'iso.BuPi' % isoluminant blue-pink
    ...
    case 'div.km.PuOr' % divergent purple-orange, by Kenneth Moreland
        ...
        interp = 'mshdiv';
    case 'div.km.GrPu' % divergent green-purple, by Kenneth Moreland
